...
public class Bank
{
  ...
  public void Bank(TimeKeeper timer)
  {
    this.timer = timer;
  }
  ...
  public void deposit(String accountNumber, int dollars, int cents) throws NoSuchAccount
  {
    Account account = getAccount(accountNumber);
    account.deposit(timer.getTime(), dollars, cents);
  }
  
  public void withdraw(String accountNumber, int dollars, int cents) throws NoSuchAccount, InsufficientFunds
  {
	  Account account = getAccount(accountNumber);
	  account.withdraw(timer.getTime(), dollars, cents);
  }
  
  public void transfer(String accountFrom, String accountTo, int dollars, int cents) throws NoSuchAccount, InsufficientFunds
  {
	  Account from = getAccount(accountFrom);
	  Account to = getAccount(accountTo);
	  from.withdraw(timer.getTime(), dollars, cents);
	  to.deposit(timer.getTime(), dollars, cents);	  
  }
  public void reportWithFilter(TransactionFilter filter, String accountNumber) throws NoSuchAccount
  {
	  Account account = getAccount(accountNumber);
	  Transactions transactions = account.filter(filter);
	  return new 
